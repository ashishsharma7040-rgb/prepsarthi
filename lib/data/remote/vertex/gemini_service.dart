...
class GeminiService {
  ...
  // ── AI Cost Control: 6-hour cooldown per report type ─────────────────────
  // Prevents students from hammering Gemini by pulling-to-refresh repeatedly.
  // SharedPreferences stores the last fetch timestamp keyed by report type.
  ...
  static Future<bool> _isCooldownExpired(String prefKey) async {
    try {
      ...
      return diff.inHours >= _cooldownHours;
    } catch (_) {
      return true; // if prefs fail, allow the call
    }
  }

  /// Records the current timestamp for a cooldown key.
  static Future<void> _stampFetchTime(String prefKey) async {
    try {
      final prefs = await _getPrefs();
  ...
  static Future<Duration> remainingCooldown(String prefKey) async {
    try {
      final prefs = await _getPrefs();
      final lastMs = prefs.getInt(prefKey) ?? 0;
  ...
  // Lazy SharedPreferences instance
...
''';
  ...
  // ── 4. Concept Connector — cross-navigation in-memory cache ─────────────
  //
  // The cache is keyed by chapter name and lives for the app session.
  // This prevents a fresh Gemini call every time the user navigates back to
  // ChapterDetailScreen, which was the source of runaway API spend.
  static final Map<String, ConceptConnectorResult> _connectorCache = {};

  static Future<ConceptConnectorResult> getConceptConnections({
    required ChapterSchema chapter,
    required List<ChapterSchema> allChapters,
  }) async {
    // Serve from cache — zero cost, instant UX
    final cached = _connectorCache[chapter.name];
...
''';
    ...
    final result = ConceptConnectorResult.fromJson(json);
    // Cache the result for this session — avoids duplicate API calls
    _connectorCache[chapter.name] = result;
    return result;
  }
...
class SWOTRecommendation {
  ...
  Map<String, dynamic> toJson() => {'day_range': dayRange, 'action': action};
}
