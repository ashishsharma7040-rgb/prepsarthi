                  ...
                  ...radar.weakChapters.asMap().entries.map((e) =>
                      _ChapterRow(
                        ...
                        onAddToPlan: () => ref
                            ...
                            .addQuickEntry(
                                e.value.name, e.value.subjectName),
...
class _ChapterRowState extends State<_ChapterRow> {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final subjColor = _subjectColor(widget.chapter.subjectName);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: widget.badgeColor.withOpacity(0.2)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Text(widget.chapter.subjectName,
                            style: TextStyle(
                                fontSize: 11, color: subjColor)),
                        Text(
                            ' · W${widget.chapter.weightage.toStringAsFixed(1)}',
                            style: theme.textTheme.labelSmall),
                        Text(
                            ' · Level ${widget.chapter.masteryLevel}/7',
                            style: theme.textTheme.labelSmall),
                      ],
                    ),
                  ],
              ...
              if (widget.onToggleWeak != null) ...[
                const SizedBox(width: 8),
                GestureDetector(
                  onTap: widget.onToggleWeak,
                  child: Icon(
                    widget.chapter.isWeakChapter
                        ? Icons.flag_rounded
                        : Icons.flag_outlined,
                    color: widget.chapter.isWeakChapter
                        ? LightColors.error
          ...
          // TASK 9: "Add to this week" button
              ...
              child: OutlinedButton.icon(
                onPressed: _adding
                    ...
                    : () async {
                        ...
                        try {
                          await widget.onAddToPlan!();
                          if (mounted) {
                            messenger.showSnackBar(
                              SnackBar(
                                content: Text(
                                  '${widget.chapter.name} added to this week\'s plan ✓',
                                ),
                ...
                label: Text(_adding ? 'Adding…' : 'Add to this week'),
                style: OutlinedButton.styleFrom(
                  foregroundColor: subjColor,
                  side: BorderSide(color: subjColor.withOpacity(0.4)),
                  padding: const EdgeInsets.symmetric(
