...
//
// ── Shanti Scholar changes (UI only, zero logic change) ──────────────────────
// • Body wrapped in SoothingBackground (gradient + dot pattern + ambient blobs)
// • SliverAppBar is fully transparent — background comes from SoothingBackground
// • AppHaptics added to theme toggle, FAB, and pull-to-refresh
// ─────────────────────────────────────────────────────────────────────────────
...
class DashboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      // ── SoothingBackground wraps the entire body ──────────────────────────
      body: SoothingBackground(                                        // ← NEW
        variant: BackgroundVariant.home,                               // ← NEW
        child: RefreshIndicator(
          onRefresh: () async {
            AppHaptics.refresh();                                      // ← NEW
          ...
          child: CustomScrollView(
            ...
            slivers: [
              // ── App Bar (transparent — SoothingBackground shows through) ─
              SliverAppBar(
                expandedHeight: 170,
                pinned: true,
                floating: false,
                backgroundColor: Colors.transparent,                   // ← CHANGED
                surfaceTintColor: Colors.transparent,                  // ← NEW
                elevation: 0,                                          // ← NEW
                ...
                actions: [
                  ...
                  A11yIconButton(
                    ...
                    onPressed: () {
                      ...
                      context.push(AppRoutes.todayMission);
                    },
                  ),
                ],
              ),
                    ...
                    delegate: SliverChildListDelegate([
                      ...
                      if (overdueRevisions > 0) ...[
                        ...
                        A11yTappable(
                          ...
                          onTap: () {
                            AppHaptics.light();                        // ← NEW
                            context.push(AppRoutes.revision);
                          },
      ...
      ),                                                               // ← closes SoothingBackground
      floatingActionButton: Semantics(
        label: 'Log a study session',
        button: true,
        child: FloatingActionButton.extended(
          onPressed: () {
            AppHaptics.buttonPress();                                  // ← NEW
