            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  ...
                  // ── 14-day Study Chart (Tier 3 Task 11) ─────────────
                  _SectionHeader(title: 'Study Trend', emoji: '📈'),
                  const SizedBox(height: 10),
                  _StudyHoursChart(
                    logs: logs,
                    targetHours: targetHours,
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: actions.asMap().entries.map((e) => Expanded(
        child: Padding(
          padding: EdgeInsets.only(right: e.key < actions.length - 1 ? 6 : 0),
          child: _QuickTile(emoji: e.value.$1, label: e.value.$2, route: e.value.$3),
        ),
      )).toList(),
    );
  }
...
// ─── Study Hours Chart (Tier 3 Task 11) ──────────────────────────────────────
// 14-day line chart. Green when above target, amber when below.
class _StudyHoursChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxY = ([...hoursPerDay, targetHours + 1]).reduce((a, b) => a > b ? a : b);
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (v, meta) {
                        if (v == 0) return Text('14d ago', style: theme.textTheme.labelSmall);
                        if (v == 13) return Text('Today', style: theme.textTheme.labelSmall);
                    ...
                    dotData: FlDotData(
                      ...
                      getDotPainter: (spot, __, ___, ____) {
                        ...
                        return FlDotCirclePainter(
                          radius: 3,
                          color: aboveTarget ? LightColors.learned : accent,
                          strokeWidth: 0,
                        );
                      },
                    ),
